...
class KirkClient(object):
    ...
    def get_user_token(self):
        ...
        if r.status_code == 200:
            ...
            _token = res['token']
            _issued_at = _token['issued_at']   # 用户登陆时间
            _expires_at = _token['expires_at'] # 用户失效时间
            _tokenid = _token['id']            # 用户Token

            self._usertoken = _tokenid
    ...
    def get_project_token(self, project_name):
        ...
        headers = {"X-Auth-Token": self._usertoken}
    ...
    def get_users_projects(self, includeType=True):
        ...
        """
        headers = {"X-Auth-Token": self._usertoken}
        r = requests.get("%s/v1/users/%s/projects?includetype=%s" % (self.host, self._userid, includeType), headers=headers)
        if r.status_code == 200:
            return r.json()
        else:
            pass
